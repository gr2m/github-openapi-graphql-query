...
function definitionToEndpoint({ method: rawMethod, url }, definition) {
  ...
  if (definition.servers) {
    const descriptionAndDefault =
      // at some point the structure of the "servers" value in `POST /repos/{owner}/{repo}/releases/{release_id}` was simplified
      // from
      //
      //     "servers": [
      //       {
      ...
      //       }
      //     ],
      "url" in definition.servers[0]
        ? {
            description: definition.servers[0].description,
            default: definition.servers[0].url,
          }
        : {
  ...
  for (const code of Object.keys(definition.responses)) {
    ...
    for (const mediaType of Object.keys(definition.responses[code].content)) {
      const { schema, example, examples } =
        definition.responses[code].content[mediaType];
